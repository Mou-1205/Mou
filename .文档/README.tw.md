...
<img align='right' src='logo.png' width='200px' alt="某炜 logo">
...
[**🖥️ 在线演示**](https://mizuki.mysqil.com/) | [**📝 文档**](https://docs.mizuki.mysqil.com/)

🌏 **README 语言：**
[**English**](./README.md) / [**中文**](./README.zh.md) / [**日本語**](./README.ja.md) / [**繁體中文**](./README.tw.md) /

通过我们的综合文档快速开始。无论是自定义主题、配置功能，还是部署到生产环境，文档涵盖了你成功启动博客所需的一切内容。
...
> **🎯 自动分辨率算法** - 根据设备屏幕分辨率智能适配内容布局，为所有设备提供最佳浏览体验
...
- **自定义特色页面：** 编辑 `src/content/spec/` 中的文件
...
- **环境变量配置（可选）：** 参照 `.env.example` 来配置
  **不建议**将 `.env` 文件提交到 Git，`.env` 应该仅在本地调试或构建使用。若要将项目在云平台部署，建议通过平台的 `环境变量` 配置传入。
...
pinned: false
comment: true
lang: zh-TW # 仅当文章语言与 config.ts 中的站点语言不同时设置
---
```
...
`pinned` 字段允许你将重要文章置顶到博客列表的顶部。置顶文章将始终显示在普通文章之前，无论其发布日期如何。
...
`comment` 字段允许你单独控制每篇文章评论区的开启与关闭。
...
某炜支持超越标准 GitHub Flavored Markdown 的增强功能：
...
- **目录：** 从标题自动生成，支持平滑滚动
- **阅读时间：** 自动计算和显示
- **文章元数据：** 丰富的 frontmatter 支持，包含分类和标签
...
某炜支持将代码和内容分成两个独立的仓库管理，适合团队协作和大型项目。
...
- **[Fuwari](https://github.com/saicaca/fuwari)** by saicaca - 本项目所基于的原始模板。感谢你创建了如此漂亮且功能强大的模板。
- **[Yukina](https://github.com/WhitePaper233/yukina)** - 感谢提供设计灵感和创意，帮助塑造了这个项目。Yukina 是一个优雅的博客模板，展现了出色的设计原则和用户体验。
- **[Firefly](https://github.com/CuteLeaf/Firefly)** - 感谢提供优秀的布局设计思路，双侧边栏布局、文章双列网格布局以及部分小部件的设计与实现，让某炜的界面更加丰富。
- **[Twilight](https://github.com/spr-aachen/Twilight)** - 感谢提供灵感和技术支持。Twilight 的动态壁纸模式切换系统、响应式设计和过渡效果显著提升了某炜的用户体验。

## 🍀 贡献者

感谢所有贡献者对本项目做出的贡献。如有问题或建议，请提交 [Issue](https://github.com/LyraVoid/Mizuki/issues) 或 [Pull Request](https://github.com/LyraVoid/Mizuki/pulls)。
